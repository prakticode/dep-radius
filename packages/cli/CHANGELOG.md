...
- 1c9b05c: A signature change that only touches optional parameters no longer flags the calls that
  stop before them. When an options object gains a property, `create()` with no arguments sees the
  same function and is left out, while `create({ name })` is still shown. The type parameters, the
  return type and the number of parameters must stay the same; a spread argument always counts.
- Updated dependencies [1c9b05c]
  - @dep-radius/core@0.4.11
...
- Updated dependencies [72e5aee]
- Updated dependencies [5ad9745]
- Updated dependencies [10e4d57]
  - @dep-radius/core@0.4.10

## 0.4.9

### Patch Changes

- baa84dc: A folder installed on its own, such as a `functions/` folder with its own `package.json`,
  is no longer read with the versions of the project around it. radius reads each manifest's nearest
...
- Updated dependencies [baa84dc]
  - @dep-radius/core@0.4.9
...
- Updated dependencies [1d9b7e5]
  - @dep-radius/core@0.4.8

## 0.4.7

### Patch Changes

- 1ad0627: A release note whose commit title names a scope and another API, such as
  `fix(model): make Model.bulkWrite() not throw`, is no longer tied to every use of `model`. The
  scope is the area the commit touched, not the change. On a mongoose 7 to 9 upgrade this cuts the
  notes listed against a project from 97 to 13, with the breaking notes still first.
- Updated dependencies [1ad0627]
  - @dep-radius/core@0.4.7
