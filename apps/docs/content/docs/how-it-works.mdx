...
`dotenv.config()` call even though the code never passes `quiet`. An option matches only when it is
written as code (`quiet`, `returnNull`) or as the header it sets (`Content-Security-Policy` for
`contentSecurityPolicy`), and these matches are marked "possibly". The types only tell which options
