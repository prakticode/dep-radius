...
description: "The --json output, field by field. Version 1 only adds fields."
icon: Braces
---

`radius --json` prints one JSON object. It has the same content as the terminal output and the pull
request comment.

## Will it change?

- The object has `"schemaVersion": 1`.
- Version 1 **only adds fields**. Removing or renaming a field would mean `schemaVersion: 2`.
- The JSON Schema is included: `@dep-radius/core/schema/brief-v1.schema.json`.

Read the fields you need and ignore the others. Your code will keep working.
...
Each entry of `breakingWithoutApi` and `changesWithoutApi` has `version`, `title` and `refs`. It can also have `likely`: a hint, never part of the verdict. The note names code inside the package (`setItem`) that changed under an export you use (`persist`), so it probably reaches your code there. Each item has:

| Field    | What                                                                  |
| -------- | --------------------------------------------------------------------- |
| `export` | The export you use whose code changed, like `schemakit:parse`          |
| `via`    | The note's names found in that changed code                           |
| `sites`  | Where your code uses the export                                       |
...
| `out-of-sync`             | review    | The lockfile or `node_modules` is out of date with `package.json` |
